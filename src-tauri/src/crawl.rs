//! Polite, bounded whole-site crawler (M1).
//!
...
pub struct CrawlProgress {
    /// `running` | `finishing` | `done` | `stopped` | `capped` | `error`.
    pub status: String,
    ...
    /// When terminal, why the crawl stopped (e.g. cap reason), else empty.
...
/// Run a crawl to completion on the calling thread (the command spawns this in
/// a background thread). Emits progress via `emit_progress` and returns the
/// final result. `stop` is polled cooperatively.
pub fn run_crawl<F>(
    cfg: CrawlConfig,
    stop: Arc<AtomicBool>,
    emit_progress: F,
    ...
    F: Fn(CrawlProgress) + Send + Sync + 'static,
{
    ...
    for _ in 0..concurrency {
        ...
        handles.push(thread::spawn(move || {
            worker(shared, http, stop, emit, out_dir, started);
        }));
    }
    for h in handles {
        let _ = h.join();
    ...
    // Finalize: rewrite inter-page links, then write pages.
    let mut guard = shared.0.lock().unwrap();
    guard.status = if guard.status == "running" { "done".to_string() } else { guard.status.clone() };
    finalize(&mut guard);
    ...
    F: Fn(CrawlProgress) + Send + Sync + 'static,
{
    ...
    loop {
        ...
        // Computed under lock, slept outside the lock.
        let (wait, delay_for_host) = {
            let mut g = lock.lock().unwrap();
            let host = url.host_str().unwrap_or("").to_string();
            let now = Instant::now();
            let rate = g.cfg.rate_per_sec.max(0.01).min(HARD_RATE_CEILING);
            ...
            (start_at.saturating_duration_since(now), effective_host_gap)
        };
        let _ = delay_for_host;
        if !wait.is_zero() {
            // Poll stop while sleeping so Stop is responsive.
            ...
            Ok(page) => {
                ...
                if let Some(page_dir) = page_dir {
                    ...
                    for asset in &page.assets {
                        ...
                        let _ = std::fs::write(&dest, &asset.body);
                    }
                }
...
fn check_caps(g: &Shared, started: Instant) -> Option<String> {
    ...
    if started.elapsed().as_secs() >= g.cfg.max_seconds {
        return Some(format!(
...
where
    F: Fn(CrawlProgress) + Send + Sync + 'static,
{
    let prog = snapshot_progress(g, &g.status, started.elapsed().as_secs());
    (emit)(prog);
}
